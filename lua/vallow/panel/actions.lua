...
M.setup = function(buf)
  ...
  map(cfg.next_section, function()
    M.move_section(buf, 1)
  end)
  map(cfg.prev_section, function()
    M.move_section(buf, -1)
  end)
  map(cfg.filter, function()
    M.filter(buf)
  end)
  map(cfg.clear_filter, function()
    M.clear_filter(buf)
  end)
  map(cfg.pick, function()
    require("vallow.picker").open(panel.state.results)
  end)

  -- Jump modes
  map(cfg.jump, function()
    M._do_jump(buf, "edit")
  end)
  map("o", function()
    M._do_jump(buf, "split")
  end)
  map("v", function()
    M._do_jump(buf, "vsplit")
  end)
  map("t", function()
    M._do_jump(buf, "tabedit")
  end)
  ...
  map("zc", function()
    M.set_fold(buf, false)
  end)
  map("zR", function()
    M.set_all_folds(buf, true)
  end)
  map("zM", function()
...
M._do_jump = function(buf, cmd)
  ...
  if item.lnum then
    pcall(vim.api.nvim_win_set_cursor, 0, { math.max(1, item.lnum), math.max(0, item.col or 0) })
    vim.cmd("normal! zz")
...
M.set_all_folds = function(buf, open)
  ...
  local secs = {}
  local cats = {}
  for key in pairs(cfg.sections or {}) do
    secs[key] = open
  end
  for key in pairs(cfg.categories or {}) do
    cats[key] = open
  end
...
M.switch_tab = function(buf, direction)
  ...
  if panel.state.results then
    require("vallow.panel.render").render(buf, panel.state.results, panel.state.win)
    tabs.set_winbar(panel.state.win, panel.state.current_section, panel.state.results, require("vallow.config").get())
  end
...
M.filter = function(buf)
  ...
  vim.keymap.set("i", "<CR>", close, { buffer = sbuf, nowait = true, silent = true })
...
M.clear_filter = function(buf)
  if (vim.b[buf].vallow_filter or "") == "" then
    return
  end
...
M.detail = function(buf)
  ...
  if item.actions and #item.actions > 0 then
    ...
    for _, act in ipairs(item.actions) do
      local text = type(act) == "table" and (act.message or act.description or act.text or vim.inspect(act))
        or tostring(act)
      push("  • " .. text, "Comment")
    end
  end
  ...
  local function close_detail()
    ...
    vim.keymap.set("n", cfg_km.close, function()
