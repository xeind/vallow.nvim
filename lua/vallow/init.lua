...
M.statusline = function()
  ...
  if state.results._loading     then return prefix .. "…" end
  if state.results.error        then return prefix .. "!" end
  if not state.results.findings then return "" end
  local total = 0
...
M.get_counts = function()
  ...
  if state.results._loading then return { total = 0, loading = true,  error = false } end
  if state.results.error    then return { total = 0, loading = false, error = true  } end
  local total = 0
  for _, b in pairs(state.results.findings) do
    if type(b) == "table" and b.count then total = total + b.count end
  end
